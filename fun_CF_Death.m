function dz_vect = fun_CF_Death(t, z, kappa, CrossFeed_Mat, Mat_kappa_3, Resource_Matrix, Threshold_CF, Threshold_death, Death_Mat, yield_Pred, S, Lag_time_Cons, Lag_time_Pred, nb_Res, Hill_CF, Hill_Pred)
z(z<0) = 0;
...
T(isnan(T)) = 0;
k = Hill_Pred;%10;
T_death = (1./(1 + (Threshold_death(1)./z(:,1)).^k));%1./(1 + (Threshold_death(1)./(max(z(:,3) - sum(r),0))).^k));%
