function dz_vect = fun_Death(t, z, kappa, CrossFeed_Mat, Resource_Matrix, Threshold, Threshold_pred, Pred_Mat, yield_Pred, S, Lag_time_Cons, Lag_time_Pred, nb_Res, Hill_CF, Hill_Pred)
z(z<0) = 0;
...
T(isnan(T)) = 0;
k = Hill_Pred;%10;
T_pred = (1./(1 + (Threshold_pred(1)./(max(z(:,3) - sum(r),0))).^k));%(1./(1 + (Threshold_pred(1)./z(:,3)).^k));%Threshold on resource concentration?
