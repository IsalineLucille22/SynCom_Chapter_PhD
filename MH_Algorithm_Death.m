...
nb_Res = 7;%9 %Number of resources (group of resources)
...
Lag_time_Pred = 0*max(normrnd(200, 10, S, S), 0);
...
Pred_Mat_Temp(16,16) = Pred_Mat(16,16);
CrossFeed_Mat_Temp(1:1+size(CrossFeed_Mat_Temp,1):end) = 0;
yield_Pred = 0.2; 

% Create a resource matrix
%Load parameters
Resource_Matrix = load(strcat('Data/', Name_file, '_Resource_Matrix_init.mat')); Resource_Matrix = Resource_Matrix.Resource_Matrix_init;
R = load(strcat('Data/', Name_file, '_R_mat_init.mat')); R = R.R_init;
Lag_time_Cons = load(strcat('Data/', Name_file, '_Lag_time_Cons_init.mat')); Lag_time_Cons = Lag_time_Cons.Lag_time_Cons_init;
x = rand(S,nb_Res);
...
while (liste_nrj_tot_temp > 1.5 || (acceptance_ratio_temp < 0.21 || acceptance_ratio_temp > 0.29) || p < 5000) &&  p < N && num_steps > 0%5.6, 0.22:0.27
    ...
    theta_Hill = 1;
    for k = 1:num_steps
        sol = ode45(@(t, y) fun_Death(t, y, kappa_mat, CrossFeed_Mat_Temp_cand, Resource_Matrix, Threshold_CF_cand, Threshold_pred, Pred_Mat_Temp, yield_Pred, S, Lag_time_Cons, Lag_time_Pred, nb_Res, Hill_CF_coeff_cand, Hill_Pred_coeff), tspan,  mat_y_0, opts_1); %Multiple resource groups
    ...
    for k = 1:num_steps
        ...
        sol = ode45(@(t, y) fun_Death(t, y, kappa_mat, CrossFeed_Mat_Temp, Resource_Matrix, Threshold_CF, Threshold_pred_cand, Pred_Mat_Temp_cand, yield_Pred, S, Lag_time_Cons, Lag_time_Pred, nb_Res, Hill_CF_coeff, Hill_Pred_coeff_cand), tspan,  mat_y_0, opts_1); %Multiple resource groups
        z_temp = deval(sol, Time_step);
        X = z_temp(mod(1:S*3,3) == 1,:); %Species'biomass
        P = z_temp(mod(1:S*3,3) == 2,:); %Complexes'biomass
        X = X + P;
        ...
        ind = eye(S,S);
        Pred_Mat_Temp_cand(~ind) = 0; %Changement pour diagonale zeros
    ...
    theta = var_theta*mean(mean(Resource_Matrix));%var_theta*mean(mu_max_dist(:,2)); % 
    for k = 1:num_steps
        
        sol = ode45(@(t, y) fun_Death(t, y, kappa_mat, CrossFeed_Mat_Temp, Resource_Matrix_cand, Threshold_CF, Threshold_pred, Pred_Mat_Temp, yield_Pred, S, Lag_time_Cons, Lag_time_Pred, nb_Res, Hill_CF_coeff, Hill_Pred_coeff), tspan,  mat_y_0, opts_1); %Multiple resource groups
        z_temp = deval(sol, Time_step);
        X = z_temp(mod(1:S*3,3) == 1,:); %Species'biomass
        P = z_temp(mod(1:S*3,3) == 2,:); %Complexes'biomass
        X = X + P;
    ...
    for k = 1:num_steps
        ...
        sol = ode45(@(t, y) fun_Death(t, y, kappa_mat, CrossFeed_Mat_Temp, Resource_Matrix_cand, Threshold_CF, Threshold_pred, Pred_Mat_Temp, yield_Pred, S, Lag_time_Cons_cand, Lag_time_Pred, nb_Res_new, Hill_CF_coeff, Hill_Pred_coeff), tspan,  mat_y_0_new, opts_1); %Multiple resource groups
        z_temp = deval(sol, Time_step);
        X = z_temp(mod(1:S*3,3) == 1,:); %Species'biomass
        P = z_temp(mod(1:S*3,3) == 2,:); %Complexes'biomass
        X = X + P;
...
sol = ode45(@(t, y) fun_Death(t, y, kappa_mat, CrossFeed_Mat_Temp, Resource_Matrix, Threshold_CF, Threshold_pred, Pred_Mat_Temp, yield_Pred, S, Lag_time_Cons, Lag_time_Pred, nb_Res, Hill_CF_coeff, Hill_Pred_coeff), tspan,  mat_y_0, opts_1); %Multiple resource groups
z_temp = deval(sol, Time_step);
X = z_temp(mod(1:S*3,3) == 1,:); %Species'biomass
P = z_temp(mod(1:S*3,3) == 2,:); %Complexes'biomass
X = X + P;
